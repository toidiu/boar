...
#
# - Chowns /boar/reports back to the host user (HOST_UID:HOST_GID, default
#   1000:1000) on shell exit, so bind-mounted output isn't root-owned on the
#   host. No-op on boar-server, where nothing writes to /boar/reports.
...
chown_reports() {
    chown -R "${HOST_UID:-1000}:${HOST_GID:-1000}" /boar/reports 2>/dev/null || true
}
