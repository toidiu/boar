...
impl DeliveryRate {
    ...
    pub fn new_from_logs(logs: &str) -> Result<Self, StatsError> {
        ...
        let matched = re
            .captures(logs)
            .and_then(|c| c.get(0))
            .ok_or_else(|| StatsError::Parse(logs.to_owned()))?
            .as_str();

        let rate = matched
            .trim_start_matches("delivery_rate=")
            .trim()
            .parse::<usize>()
            .map_err(|_| StatsError::Parse(logs.to_owned()))?;

        Ok(DeliveryRate { rate })
    }
}
...
mod tests {
    ...
    fn delivery_rate() {
        ...
        let metric = DeliveryRate::new_from_logs(logs).unwrap();
